...
LLPoolEntry *LLPoolAddItem( LLPool *pool, void *item )
{
  ...
  if( pool->free ) {
    ...
    entry = malloc( sizeof( LLPoolEntry ) + pool->dataSize );
    assert( entry != 0 );
  }
...
{
  Connection *conn = LLPoolGetItem( connEntry );
...
void startDealer( const Config *conf, const Match *match, MatchJob *job )
{
  ...
  char handsString[ 16 ], rngString[ 16 ], timeoutString[ 16 ];
  ...
  if( !job->dealerPID ) {
    ...
    if( conf->startupTimeoutSecs ) {
      ...
      sprintf( timeoutString, "%d", (int)conf->startupTimeoutSecs * 1000 );
      argv[ arg ] = timeoutString;
      ++arg;
    }
		      ...
		      const uint32_t rngSeed )
{
  ...
  for( p = 0; p < match->gameConf->game->numPlayers; ++p ) {
    ...
    if( match->players[ p ].isNetworkPlayer ) {
      ...
      Connection *conn = LLPoolGetItem( match->players[ p ].entry );
...
int startMatchJob( const Config *conf, ServerState *serv )
{
  ...
  for( cur = LLPoolFirstEntry( serv->jobs );
    ...
    running += botsInMatch( LLPoolGetItem( job->matchEntry ) );
  }
